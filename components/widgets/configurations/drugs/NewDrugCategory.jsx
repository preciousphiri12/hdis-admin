...
export default function NewDrugCategory({
  openDrugCategory,
  setOpenDrugCategory,
  newDrugCategory,
}) {
  ...
  const saveHandler = () => {
    let isvalid = Validations.validate([category.name, category.description]);
    if (isvalid) {
  ...
  return (
    <div className="">
      <Transition.Root show={openDrugCategory} as={Fragment}>
        <Dialog
          as="div"
          className="relative z-10"
          onClose={setOpenDrugCategory}
        >
          <Transition.Child
            as={Fragment}
                ...
                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                  <div className="absolute right-0 top-0 hidden pr-4 pt-4 sm:block">
                    <button
                      ...
                      onClick={() => setOpenDrugCategory(false)}
                    >
